...
- **参数**在正偏移一侧，从最后一个参数往前数：最后一个参数固定在 `[rbp+16]`，往前每多一个参数偏移 +8，第一个参数在最大偏移处。参数统一占 8 字节一格（不管声明类型是什么，调用方总是按 8 字节 push），用 `%define` 给整个内存操作数起别名，例如 `%define flag [rbp + 16]`。**硬性规定：所有函数（subroutine）的每一个参数都必须恰好是 8 字节**——要么是一个能装进 8 字节的普通标量值（整数、指针……），要么如果要传结构体，只能传"指向该结构体的指针"，绝不把结构体按值展开成多个栈槽传递。这样参数区的布局才能保持"每个参数固定 8 字节、`[rbp+16+8k]` 规律排列"这条简单规则，不需要为某个参数特殊照顾。
- **局部变量**在负偏移一侧，但**不再假定每个变量固定占 8 字节**——每个局部变量按自己的实际大小（`int32_t` 占 4 字节、结构体占 `sizeof` + `CamelCase` 类型名 字节……）挨着上一个变量的末尾继续往负方向排。因此局部变量不再用 `%define` 起"整个内存操作数"的别名，而是用 `%assign` 给它起一个**纯数值偏移**的别名，访问时自己写 `[rbp + 变量名]`（结构体成员则是 `[rbp + 变量名 + 类型名_字段名]`）。这样才能表达"这个变量到底占几个字节"，也才能正确处理结构体这种非 8 字节整数倍大小的变量。具体写法见下面「结构体字段偏移约定」和「局部变量偏移约定」两节。
...
有了这套偏移常量，不管这个结构体实例是全局变量、堆内存，还是接下来要讲的栈上局部变量，访问某个字段都统一是「基址 + 字段偏移」，比如 `[rbp + c + Sample_y]`（`c` 是栈上局部变量 `Sample` 的起始偏移）或者 `[some_ptr + Sample_y]`（`some_ptr` 是指向堆/全局 `Sample` 实例的指针，一般也就是作为参数传进来的那个结构体指针）。

### 局部变量偏移约定

局部变量同样用 `%assign` 表达偏移，而不是 `%define` 整个内存操作数：从「`-`（第 1 个变量占用的字节数）」开始，之后每多声明一个变量，偏移再往负方向减去**上一个变量占用的字节数**。
...
```asm
%assign a (-8)                ; a 是 int32_t，只用 4 字节数据，但仍按 8 字节整槽分配
%assign b (a - 8)             ; b 同理
%assign c (b - sizeofSample)    ; c 是 struct Sample，sizeofSample 已经是 8 的整数倍（16）
%assign stksz (-c)            ; 天然是 8 的整数倍，不需要再单独取整
sub rsp, stksz
```
...
assert:
    ...
    begin
    ;; local vars
    %assign result (-8)  ; 第 1 个局部变量，8 字节，通过 [rbp + result] 访问
    sub  rsp, 8            ;; stksz=8，已是 8 的倍数，N=8

    ; ... 用 [rbp + result] 存中间结果 ...
    mov rax, [rbp + result] ;; rax 是返回值

    end
    ret  16    ; 清理调用方压入的 2 个参数（16 字节），局部变量随 end 里的 mov rsp,rbp 一并归还
```

单参数、无局部变量的函数（如下面例子里的 `fibo`、`printNum`）参数恒定落在 `[rbp+16]`；没有局部变量时 `stksz=0`，`sub rsp` 直接省略。
...
fibo:
    ...
    begin
    ;; local vars
    %assign acc (-8)   ; 用来存第一次递归调用的结果，8 字节
    sub  rsp, 8          ; stksz=8，已是 8 的倍数，N=8
