...
也就是说，参数偏移要**从最后一个参数往前数**：最后一个参数固定在 `[rbp+16]`，往前每多一个参数偏移 +8，第一个参数在最大偏移处。

不过写 `%define` 别名时，书写顺序仍然按**参数声明顺序**（第一个参数写在最前面），不用跟着偏移倒序写 —— 只是每个别名对应的具体偏移值要按上面的规则算，不是从 `[rbp+16]` 依次往上排。函数体内直接写别名即可读取，可读性接近高级语言。
